...
class CollateraldutyTest < ActiveSupport::TestCase
  def setup
  ...
  test "less_than_four_hours" do
    m = Member.create(:firstname => 'Bob',
                      :lastname => 'Jones',
                      :badgeno => '532103')
    t = Collateralduty.new(:start_time => Time.local(2010, 3, 20, 7, 0, 0),
                    :end_time => Time.local(2010, 3, 20, 11, 0, 0),
                    :member_id => m.id)
    assert(t.save)
    assert_equal(1, t.points)

    t2 = Collateralduty.new(:start_time => Time.local(2010, 3, 21, 7, 0, 0),
  ...
  test "next_morning" do
    m = Member.create(:firstname => 'bob', 
                      :lastname => 'jones', 
                      :badgeno => '532103')
    sb = Collateralduty.new(:start_time => Time.local(2010, 2, 13, 18, 0, 0),
  ...
  test "points" do
    ...
    sb = Collateralduty.new(:start_time => Time.local(2010, 4, 13, 7, 0, 0),
  ...
  test "no_overlap" do
    ...
    sb1 = Collateralduty.new(:start_time => start1,
                      :end_time => end1,
                      :member_id => m.id)
    assert(sb1.save, sb1.errors.inspect)
    
    sb2 = Collateralduty.new(:start_time => start2,
                      :end_time => end2,
                      :member_id => m.id)
  ...
  test 'no_overlap_with_sleep_in' do
    ...
    sb = Collateralduty.new(:start_time => Date.today - 30.days + 13.hours,
    ...
    sb = Collateralduty.new(:start_time => Date.tomorrow - 30.days + 8.hours,
                     :end_time => Date.tomorrow - 30.days + 12.hours)
    sb.member = m1
    assert(sb.save)
    assert(sb.destroy)
    
    sb = Collateralduty.new(:start_time => Date.tomorrow - 30.days + 7.hours,
                     :end_time => Date.tomorrow - 30.days + 11.hours)
    sb.member = m1
    assert(sb.save)
    assert(sb.destroy)
    
    sb = Collateralduty.new(:start_time => Date.tomorrow - 30.days + 3.hours,
    ...
    sb = Collateralduty.new(:start_time => Date.tomorrow - 30.days,
                     :end_time => Date.tomorrow - 30.days + 5.hours)
    sb.member = m1
    assert(!sb.save)

    sb = Collateralduty.new(:start_time => Date.today - 30.days + 17.hours,
                     :end_time => Date.today - 30.days + 19.hours)
    sb.member = m1
    assert(sb.save)
  ...
  test 'no_future_months' do
    sb = Collateralduty.new(:start_time => Date.today,
                     :end_time => Date.today + 4.hours,
                     :member_id => @m1.id)
    assert(sb.save)
    sb.destroy

    sb = Collateralduty.new(:start_time => Date.today.end_of_month,
                     :end_time => Date.today.end_of_month + 4.hours,
                     :member_id => @m1.id)
    assert(sb.save)
    sb.destroy

    sb = Collateralduty.new(:start_time => Date.today.next_month.beginning_of_month,
  ...
  test 'dates' do
    sb = Collateralduty.new(:start_date => '2009-04-15',
                     :end_date => '2009-04-15',
    ...
    sb = Collateralduty.new(:start_date => '2009-04-16',
                     :end_date => '2009-04-17',
  ...
  test 'unlocked' do
    sb = Collateralduty.new(:start_time => Time.local(2010, 6, 15, 7, 0, 0),
                     :end_time => Time.local(2010, 6, 15, 16, 0, 0),
                     :member_id => @m1.id)
    assert(sb.save, sb.errors.inspect)
    
    sb = Collateralduty.new(:start_time => Time.local(2010, 5, 16, 7, 0, 0),
